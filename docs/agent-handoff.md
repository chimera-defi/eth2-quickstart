...
- The validator inventory now normalizes keystore pubkeys before beacon queries, batches validator lookups in chunks of 100, and includes freshness metadata in JSON (`generated_at_utc`, `beacon_query_status`).
- `validator-manage` now validates consolidation source/target pubkeys against the local inventory before sending a transaction, uses a temporary keystore instead of `--private-key` for the withdrawal signer, and resolves Prysm voluntary-exit RPC via `PRYSM_BEACON_RPC_PROVIDER` instead of hardcoding `127.0.0.1:4000`.
- Added a dedicated `./scripts/eth2qs.sh validator-create-0x01` helper so standard and compounding validator creation are both explicit and operator-friendly.
- `validator-withdrawal-changes` writes a staging manifest and refuses to submit stale or mismatched JSON artifacts without a matching generate step.
...
- Repo guidance updates:
  - tightened the validator management docs for dry-run usage, inventory freshness, manifest-gated submit, and consolidation validation
...
- The local validator inventory now shows withdrawal credential type so agents can see `0x00`, `0x01`, and `0x02` at a glance before taking action, and Prysm inventory discovery falls back to the wallet v2 accounts listing when direct keystore parsing is insufficient.
...
## Validator flow learnings

- When a shell helper needs to be sourced in a regression test, it must be source-safe: wrap the top-level option parser and the `main` entrypoint so sourcing does not consume the caller's positional parameters.
- For voluntary exit flows, validate and normalize the selected local validators to pubkeys before the destructive confirmation prompt. That makes the operator preview accurate and prevents accidental confirmation on an invalid selection.
