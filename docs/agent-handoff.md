...
## Update: 2026-03-05 (Dead-Code Reality Check)
- Author: codex
- Why:
  - User requested adversarial verification that code is actually used and to cut non-working/non-used code.
- Findings:
  - `install/utils/optional_tools.sh` had zero call-sites across scripts, docs, tests, and workflows.
  - `install/templates/install_template.sh` was an unused scaffold template with zero call-sites.
  - `frontend/app/loading.tsx` and `frontend/app/not-found.tsx` are used by Next.js file-system conventions (no direct symbol imports expected), so they were retained.
- Changes:
  - Removed dead files:
    - `install/utils/optional_tools.sh`
    - `install/templates/install_template.sh`
- Validation:
  - `./test/run_tests.sh --full` passed (`272 passed, 0 failed`).
  - `./scripts/pre-commit.sh` passed.
- Follow-ups:
  - Repeat this scan on utility/readme mismatches (docs claiming scripts that no longer exist) in a separate small PR.
