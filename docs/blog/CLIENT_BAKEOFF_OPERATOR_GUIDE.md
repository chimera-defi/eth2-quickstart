...
*Curious how we measured all this — and the forensic detail behind the ethrex cliff and the besu deadlock? See the [technical deep-dive companion](CLIENT_BAKEOFF_DEEP_DIVE.md).*

---

*Methodology in brief: one shared semi-prod host, sequential (one client at a time), MEV off, no validator keys, 72h cap per candidate, footprint = final synced datadir (never the peak). EL scorecard holds the CL constant at prysm; CL scorecard holds the EL constant (ethrex, then geth). The `nimbus_eth1` execution-client full-sync row is a partial pending its terminal measurement (~2026-07-14) — it's a full-sync-only client that won't reach tip inside the cap, so it lands as a client-limitation footnote, not a ranking entry.*
