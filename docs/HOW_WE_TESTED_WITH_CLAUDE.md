...
The one-line takeaway from the numbers: **nobody won on disk — every EL that keeps full post-merge history converges to ~1.0–1.2 TiB, so footprint measures history-retention config rather than client efficiency; ethrex won on speed (~2h16m) but pays it back on every restart; and the consensus layer is effectively solved — all five CLs checkpoint-synced in ~22 minutes with zero crashes.** (Two candidates needed a discarded-and-rerun attempt along the way — a JVM-OOM-poisoned teku run and a harness `du`-pipeline bug on grandine's first try — neither was a CL failure.) The [results post](CLIENT_BAKEOFF_BLOG.md) has the why.
