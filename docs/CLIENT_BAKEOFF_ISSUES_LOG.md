...
### D4. prysm — experimental `enable-discovery-reboot` self-deadlocks discovery under PeerDAS → silent sync stall ⚠️
- **Symptom:** on a long-running node, prysm logged `ERROR p2p: Could not reboot listener error=create local node: could not open node's peer database: resource temporarily unavailable` every ~60s (1000+ occurrences over days), alongside `WARN sync: Failed to fetch data column sidecars` with `initialMissingRootCount == finalMissingRootCount` (zero columns fetched per attempt). `sync_distance` widened instead of closing. The paired nethermind EL received `engine_newPayload` (so `highestBlock` tracked the chain tip) but never `engine_forkchoiceUpdated` (so `currentBlock` froze) — presenting downstream as "not receiving fork choices." Systemd showed the unit `active (running)` with `NRestarts=0` the whole time.
- **Root cause:** `enable-discovery-reboot: true` in `configs/prysm/prysm_beacon_conf.yaml` (and `prysm_beacon_sync_conf.yaml`). This experimental flag periodically reboots prysm's discovery-v5 listener, which re-opens the discovery LevelDB — but the **same live PID still holds that database's `LOCK`** (`/proc/<pid>/fd/… -> ~/.eth2/discovery/LOCK`), so `flock()` `EAGAIN`s and the reboot fails every cycle. A self-deadlock, not a stale lock from a dead process. Degraded discovery can't cover the PeerDAS data-column subnets, so initial-sync starves for column sidecars and the head diverges. Ruled out: disk (24% used) and FD exhaustion (only ~57 open FDs).
- **Fix:** remove the flag (return to prysm's default of not rebooting discovery) and restart the beacon node. A bare restart alone does **not** fix it — the flag simply re-arms the deadlock. After removal: reboot-listener errors → **0**, column fetches resolved (`All needed origin data column sidecars are already present`), peers recovered (3 → 33), and `sync_distance` began falling (1119 → 1029 → 971 → 848…). Removed from both repo config templates so future installs don't inherit it.
- **Takeaway (humans):** don't add experimental "reliability" flags speculatively — `enable-discovery-reboot` *sounds* protective but backfires under PeerDAS, and prysm's default (no reboot) is safe. If prysm repeats `could not open node's peer database: resource temporarily unavailable`, it's fighting *itself* for the discovery DB lock; check for this flag before suspecting disk or file-descriptor limits.
- **Takeaway (agents):** `NRestarts=0` + `active (running)` is **not** a health signal — this wedge never crashes the process, it degrades sync while the unit stays green. It's the CL-side sibling of D3: detect it by the log signature + `sync_distance` trend + the EL `currentBlock`-frozen-while-`highestBlock`-climbs split, never by systemd state alone.
