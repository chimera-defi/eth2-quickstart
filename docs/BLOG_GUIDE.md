...
Wrap each diagram in a `<figure>` with a full-sentence `aria-label` and a `<figcaption>`; reserve `aria-labelledby`/`aria-describedby` with `<title>`/`<desc>` for real `<svg>` charts. Status colours are a shared three-tone scale — `#a855f7` good, `#f5b46b` caution, `#e5726e` failed — used categorically; the `#e9d5ff → #9333ea` purple ramp is for parts-of-one-whole only.
