...
_Final recommendation and operational-viability synthesis follows the Stage B results, below._
...
All five CLs **checkpoint-synced to a fully validating head in ~22–23 min**, `config_optimal=yes`, `anchor_synced=yes`, `service_crash_observed=no`. Sync **time** is effectively tied (checkpoint sync dominates), so **the CL datadir footprint is the differentiator** (figures are `du -sh` binary units — MiB/GiB).

| CL | Result | Sync time | CL datadir footprint | Disk-optimal lever |
| --- | --- | --- | --- | --- |
| **lighthouse** | ✅ synced | ~22m | **773,282,157 B (~739 MiB)** ← **smallest** | `checkpoint-sync-url` (blob-prune default) |
...
| **teku** | ✅ synced (`anchor_synced=no` — watchdog false positive, see caveat 2) | ~10m07s | **875,146,169 B (~835 MiB)** *(clean re-read; a first run measured ~667 MiB — see caveat 2)* | `data-storage-mode=minimal` |
...
**nethermind-anchor CL disk ranking (actual disk, smaller = better): lodestar (~178 MiB) < lighthouse (~470 MiB) < grandine (~730 MiB actual) < teku (~835 MiB) < nimbus (~1.3 GiB).**
...
- **There is no disk winner — the field converges.** Every EL that carries full post-merge history lands at ~1.0–1.2 TiB (geth 1.13, nethermind ~1.06, besu 1.08, reth ~1.1–1.2 projected); on-disk size is set by history-retention config, a client-agnostic knob, not by client efficiency. **Update 2026-08-03:** the shipped installer now defaults nethermind to minimal-history (`NETHERMIND_FULL_HISTORY=false`), landing at ~250–280 GiB — the ~1.06 TiB figure above is the full-history configuration. So pick on the axes that actually differ: **snap-sync speed** and **restart-resume stability.**
