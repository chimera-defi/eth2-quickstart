...
### CL matrix — third-anchor confirmation (anchor = **nethermind**, run_id `client-bakeoff-anchor-nethermind-2026-07-26b`, 2026-07-26)
...
| Consensus | Sync status | Sync time | Final CL datadir | History-prune lever |
|-----------|-------------|-----------|------------------|---------------------|
| **lodestar** | ✅ synced | ~7m36s | **186,083,466 B (~178 MiB)** ← **smallest** | `pruneHistory=true` |
...
| **teku** | ✅ synced | ~10m07s | **875,146,169 B (~848 MiB)** | `data-storage-mode=minimal` |
| **nimbus** | ✅ synced | ~10m13s | **1,337,611,316 B (~1.3 GiB)** ← **largest** | `history=prune` |

**nethermind-anchor CL disk ranking (actual disk, smaller = better): lodestar (~178 MiB) < lighthouse (~470 MiB) < grandine (~730 MiB) < teku (~848 MiB) < nimbus (~1.3 GiB).**
...
- **Absolute footprints scale with observation time, not just the EL anchor.** The geth-anchor and nethermind-anchor numbers are much smaller (nimbus ~1.2–1.3 GiB vs ~5.0 GB; teku ~848 MiB–936 MiB vs ~2.1 GB) than the ethrex-anchor numbers because those runs were measured minutes after checkpoint-sync (fresh datadirs, ~7–10 min for geth/nethermind vs ~22–23 min for ethrex), while the ethrex-anchor runs had filled more of the blob-retention / state-history window. The 2/2/1 tiers are anchor-independent here; exact within-tier order is measurement-window-sensitive.
