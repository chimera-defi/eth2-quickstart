...
| besu__prysm | ⚠️ **limitation** (un-pruned; pruned re-run abandoned) | ~19h18m (un-pruned run) | **~1.08 TiB (un-pruned)** — besu 1,189,836,723,674 B + prysm 1,682,488,084 B | The 2026-06-30 run snap-synced cleanly to a validating head (~50 peers, prysm `is_optimistic=false` at 2026-07-01T01:37:10Z → ~19h18m, fully_synced=yes) but ran **without history pruning** → its 1.08 TiB is history-inflated and **NOT pruned-comparable** to geth/nethermind. A pruned re-run (`history-expiry-prune=true`, started 2026-07-04) was attempted to get a comparable number; it **deadlocked twice and was abandoned** (operator: "Stop; accept limitation note", 2026-07-05 — see the besu snap-sync deadlock gotcha below). besu is therefore shown as a **client-limitation** (un-pruned 1.08 TiB), excluded from the pruned-comparable ranking. |

_Remaining (one at a time): **ethrex** (× prysm); then geth × lighthouse, teku, nimbus, lodestar, grandine._

### Client limitations — shown separately, NOT in the pruned-comparable ranking

Only **geth** (~1.13 TiB) and **nethermind** (~251 GiB) produced a final footprint under a **pruned, apples-to-apples** config, so only those two are ranked head-to-head on disk. The rest are recorded here with the reason each falls outside that comparison:

| EL | Footprint recorded | Why it's outside the pruned-comparable ranking |
| --- | --- | --- |
| besu | ~1.08 TiB (un-pruned) | Un-pruned run is history-inflated; the pruned re-run to make it comparable deadlocked twice and was abandoned (stale-pivot → `SnapSyncChainDownloader` thread death; root-caused to a ~28h prysm-v7.1.5 CL stall — see gotcha below). |
| reth | ~0.98 TiB partial (72h-capped) | `--full`-only (no snap); sequential full block execution can't finish mainnet inside the 72h cap. Speed-bound, not config-bound. |
| nimbus_eth1 | ~21 GB partial | Full-sync-only (no snap); era1 import path. Service exited ~21.6h in at ~block 2.5M/25.4M — never near tip. |
| erigon | ~1.21 TiB frozen partial | Structural no-sync: erigon3 OtterSync + checkpoint-synced-prysm optimistic gap-close deadlock. Not a synced datadir. |
| ethrex | not yet run | No history-prune lever exists (`--syncmode {full,snap}` only) → will show a full-history footprint (optimal-by-absence). |
