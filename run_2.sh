...
if [[ "$SKIP_DEPS" != "true" ]]; then
    log_info "Installing user-level dependencies (Phase 2)..."
    if ! "$SCRIPT_DIR/install/utils/install_dependencies.sh" --phase2; then
        log_error "Failed to install user-level dependencies"
        exit 1
    fi
fi
