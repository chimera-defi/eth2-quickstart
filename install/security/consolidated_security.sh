...
#
# Docker divergence (documented): Firewall excludes 172.16.0.0/12 from private-network
# blocks (bridge 172.17.0.1); all other blocks applied for proper E2E hardening.
...
setup_firewall() {
    ...
    # In Docker: exclude 172.16.0.0/12 (Docker bridge 172.17.0.1); apply all other blocks for proper E2E hardening.
    # On real servers: apply full rules including 172.16.0.0/12.
    ...
    # Define private network ranges to block (Hetzner expected strict outbound blocks from Erigon README)
    private_networks=(
    ...
    # In Docker, skip 172.16.0.0/12 (container gateway 172.17.0.1); on real servers include it
    if ! is_docker; then
        private_networks+=("172.16.0.0/12")  # Private-Use Networks
    else
        log_info "Docker: excluding 172.16.0.0/12 (bridge) for connectivity; all other blocks applied"
    fi
