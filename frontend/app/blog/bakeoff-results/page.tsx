...
const clientLimitations = [
  ...
  { el: 'ethrex', footprint: '~286 GiB at sync → **~467 GiB (2026-07-06, still growing even at tip)** — 306,564,007,339 B at sync; live 500,952,726,301 B (`eth_syncing=false`, ~10 GiB/hr)', synced: 'yes', syncedDetail: '(~2h16m, fully validated)', why: 'ethrex **synced cleanly and fastest in the field (~2h16m snap).** No history-prune lever (`--syncmode snap` only; no state-prune flag). Footprint is un-pruned and **NOT full-history** — it serves ~no history (`eth_getBlockByNumber` `null` below head, verified 2026-07-06) yet keeps growing **even at the chain tip with `eth_syncing=false`** (286 → ~467 GiB, ~10 GiB/hr), so we\'re re-measuring whether ethrex\'s at-tip growth settles near the ~1.0–1.2 TiB the other ELs reach (normal post-merge backfill) or keeps climbing unbounded (a real defect); result pending (run `client-bakeoff-ethrex-steadystate-2026-07-28`). config_optimal=yes (snap is optimal-by-absence; 1 stale-pivot auto-healed). service_crash_observed=no.' },
]
          ...
          <ul className="mt-3 space-y-3 text-sm text-muted-foreground">
            ...
            <li>
              ...
              <ul className="mt-2 space-y-2 pl-4">
                <li>
                  <Rich text="**ethrex** — fastest cold sync in the whole field (~2h16m), but the restart-cliff is a real operational weakness: a 26-minute/132-block downtime gap stalled instead of resuming, and measured 1.5–2-hour gaps triggered a full ~2-hour re-snap. Separately, its datadir kept climbing at tip (286 → ~467 GiB, ~10 GiB/hr, measured 2026-07-06) — we're re-measuring whether that settles near the ~1.0–1.2 TiB the other ELs reach (normal post-merge backfill) or keeps climbing unbounded (a real defect); result pending. Snap speed is a trap if the restart cliff isn't fixed — fast to stand up, painful to *operate*. Young client (v19.0.0); may improve." />
                </li>
                <li>
