...
const recommendationPoints = [
  "Execution client: **there is no disk winner — the field converges.** Every EL that carries full post-merge history lands at ~1.0–1.2 TiB (geth 1.13, nethermind ~1.06, besu 1.08, reth ~1.1–1.2 projected); on-disk size is set by history-retention config, a client-agnostic knob, not by client efficiency. So pick on the axes that actually differ: **snap-sync speed** and **restart-resume stability.** **geth** — conservative default, largest ecosystem, cleanest ~8h28m snap, and resumes gracefully after downtime (imports missed blocks, keeps its datadir). **nethermind** — a strong minority-client pick (improves diversity; compact flat-storage state) whose resume is now directly measured and bisected (2026-08-01→03: every gap from 12 min to ~35h resumed by plain block import — no re-snap, no cliff). **ethrex** — fastest cold sync in the field (~2h16m on v19.0.0; a later re-sync took 4h09m56s on v22.0.0 — different ethrex versions and different day/host state, not a regression trend) but a ~25-min restart cliff (longer gaps trigger a full re-snap); its datadir plateaus at ~470 GiB, smaller than the pack only because it retains no history at all — a no-history node, not a pruned-comparable one, so this is not a disk win. besu snap-synced cleanly (~1.08 TiB) but is fragile to a prolonged CL outage; reth and nimbus_eth1 are full-sync-only (multi-day, capped partial here); erigon deadlocked against checkpoint-synced prysm on this host.",
