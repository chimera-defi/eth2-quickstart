...
[Service]
User            = $(whoami)
ExecStart       = /usr/bin/geth --cache 4096 --http --http.addr "127.0.0.1" --http.corsdomain "*" --http.port "8545" --http.api "db, eth, net, web3, personal, engine,admin" --ws --ws.port 8546 --ws.addr "127.0.0.1" --ws.origins "*" --ws.api "web3, eth" --maxpeers=100 --authrpc.vhosts="localhost" --authrpc.jwtsecret=$(echo $HOME)/secrets/jwt.hex --http.vhosts=*
Restart         = on-failure
RestartSec      = 3
TimeoutSec      = 300
